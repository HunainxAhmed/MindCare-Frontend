...
export default function SettingsModal({ isOpen, onClose, token, onLogout, sessions, setSessions, isDarkMode, toggleDarkMode }) {
  ...
  const fetchProfile = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000'}/api/user/profile`, {
  ...
  const handleDeleteAllChats = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000'}/api/sessions/all`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
